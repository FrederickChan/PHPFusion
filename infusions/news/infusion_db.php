...
if ( infusion_exists( 'news' ) ) {
    ...
    function news_user_action_hook( $action, $user_id ) {

        if ( $action == 'delete_user' ) {
            dbquery( "DELETE FROM " . DB_NEWS . " WHERE news_name=:uid", [':uid' => $user_id] );

            $result = dbquery("SELECT submit_criteria FROM ".DB_NEWS." WHERE submit_type=:submit AND submit_user=:uid", [
                ':uid' =>$user_id,
                ':submit' => 'n'
            ]);
            if ( dbrows( $result ) ) {
    ...
    fusion_add_hook( 'fusion_user_action', 'news_user_action_hook', 10, [], 2 );
}
