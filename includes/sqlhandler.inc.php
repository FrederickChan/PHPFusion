...
function dbquery_insert($table, array $inputdata, $mode, $options = []) {
    ...
    if (!fusion_safe() && $options['allow_remote'] === FALSE) {
        if ($options['debug']) {
...
/**
 * Update system core table
 * @param $value - table name with prefix (i.e. DB_USERS, DB_SETTINGS, etc)
 */
function fusion_update_table($value) {
    $update = PHPFusion\Installer\Batch::getInstance()->getUpdates($value);
    if (!empty($update)) {
        print_p($update);
        foreach($update as $cmd) {
            dbquery($cmd);
        }
