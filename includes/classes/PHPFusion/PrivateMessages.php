...
class PrivateMessages {
    ...
    public static function getPmSettings( $user_id, $key = NULL ) {
        if ( iMEMBER ) {
            ...
            $settings = [
                ...
                'user_pm_email'     => fusion_get_settings( 'pm_email_notify' ),
            ...
            if ( $user_id !== $userdata['user_id'] ) {
                ...
                $result = dbquery( "
                    ...
                    WHERE u.user_id=:uid AND user_status=:status", [':uid' => $user_id, ':status' => '0']
                );
                if ( dbrows( $result ) ) {
                    $data = dbarray( $result );
                    ...
                    $settings = [
                        ...
                        'user_pm_email'     => ( !empty( $data['user_pm_email'] ) ? (int)$data['user_pm_email'] : (int)$settings['user_pm_email'] ),
                ...
                $settings = [
                    ...
                    'user_pm_email_notify' => $userdata['user_pm_email'],
    ...
    private function setListMessages() {
        ...
        if ( $totals[$this->current_folder] > 0 ) {

            add_to_title( $this->locale['global_201'] . $this->info['folders'][$this->current_folder]['title'] );
            set_meta( "description", $this->info['folders'][$this->current_folder]['title'] );

            $sql_table = DB_MESSAGES . " m LEFT JOIN " . DB_USERS . " u ON (m.message_from=u.user_id)";
            $sql_condition = "message_to=:uid AND " . $query[$this->current_folder][1];
            ...
            if ( $this->info['max_rows'] = dbcount( "(message_id)", $sql_table, $sql_condition, $sql_param ) ) {
                ...
                $sql_param += [
                    ':rowstart' => get_rowstart( 'rowstart', $this->info['max_rows'] ),
                    ':limit'    => 20
                ];

                $result = dbquery( "SELECT m.*, u.user_id, u.user_name, u.user_status, u.user_avatar, u.user_level, MAX(m.message_id) AS last_message
                                FROM " . $sql_table . "
                                WHERE " . $sql_condition . " GROUP BY message_id ORDER BY m.message_datestamp DESC LIMIT " . $sql_limit, $sql_param );
                ...
                if ( $this->info['max_rows'] > $this->info['rows'] ) {
                    ...
                    $this->info['pagenav'] = makepagenav( $sql_param[':rowstart'], $sql_param[':limit'], $this->info['max_rows'], 3, BASEDIR . "messages.php?folder=" . $this->current_folder . "&" );
                }
                ...
                while ( $rows = dbarray( $result ) ) {
                    ...
                    $data['contact_user'] = [
                        ...
                        'user_addname'   => $rows['user_addname'],
                    ...
                    $data['message'] = [
                        'link'           => BASEDIR . "messages.php?folder=" . $this->current_folder . "&msg_read=" . $data['message_id'],
    ...
    private function setReadMessages() {
        ...
        $sql_table = DB_MESSAGES . " m LEFT JOIN " . DB_USERS . " u ON (m.message_from=u.user_id)";
        $sql_condition = "message_to=:uid AND message_id=:mid AND " . $query[$this->current_folder][1];
        ...
        if ( $this->info['rows'] = dbrows( $result ) ) {
            ...
            $this->info['button'] += [
                'back' => ['link' => BASEDIR . "messages.php?folder=" . $this->current_folder, 'title' => $this->locale['back']],
            ];
    ...
    public function server() {
        ...
        $this->current_folder = get( 'folder' );
        ...
        $this->info = [
            ...
            'folders'        => [
                'inbox'   => ['link'   => BASEDIR . "messages.php?folder=inbox",
                              'title'  => $this->locale['402'],
                              'active' => $this->current_folder == 'inbox' or !$this->current_folder,
                              'icon'   => get_image( 'inbox', '', '', $this->locale['402'], 'class="me-1"' )],
                'outbox'  => ['link'   => BASEDIR . "messages.php?folder=outbox",
                              'title'  => $this->locale['403'],
                              'active' => $this->current_folder == 'outbox',
                              'icon'   => get_image( 'outbox', '', '', $this->locale['402'], 'class="me-1"' )],
                'archive' => ['link'   => BASEDIR . "messages.php?folder=archive",
                              'title'  => $this->locale['404'],
                              'active' => $this->current_folder == 'archive',
                              'icon'   => get_image( 'archive', '', '', $this->locale['402'], 'class="me-1"' )],
                'options' => ['link'   => BASEDIR . "messages.php?folder=options",
                              'title'  => $this->locale['425'],
                              'active' => $this->current_folder == 'options',
                              'icon'   => get_image( 'settings', '', '', $this->locale['402'], 'class="me-1"' )],
            ],
