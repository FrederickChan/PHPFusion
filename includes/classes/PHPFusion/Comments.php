...
class Comments {
    ...
    private static $avatar_size = '32px';
    /**
     * @var array
     * comment_item_type -
     * comment_db -
                ...
                if ($can_post) {
                    ...
                    $message_input = form_textarea($edata['comment_cat'] ? 'comment_message_reply' : 'comment_message', '', $edata['comment_message'],
                        [
                            ...
                            "tinymce" => "simple",
                            'wordcount' => TRUE,
                            'placeholder' => 'What\'s on your mind, ' . display_name($userdata, FALSE, FALSE) . '?',
...
//                'comments' => $comments, // only this is being used.
...
//print_p($comments);
            ...
            $html .= display_comments_ui([
                ...
                'comment_posts' => (!empty($this->c_arr['c_con']) ? $this->displayAllComments($this->c_arr['c_con'], 0, $this->getParams()) : $no_comments_text),
     ...
     */
    protected function parseCommentsData($row, $i) {
        ...
        // Reply Form
        //(isset($_GET['comment_reply']) && $_GET['comment_reply'] == $row['comment_id'])
        ...
        $row = [
            ...
            "i" => $i,
            'user_avatar_display' => isnum($row['comment_name']) ? display_avatar($row, self::$avatar_size) : display_avatar([], self::$avatar_size),
    ...
    protected function commentsQuery($cat_id = 0) {
        $comment_query = "
                    SELECT c.* " . ($this->getParams('comment_allow_ratings') && fusion_get_settings('ratings_enabled') ? ", r.rating_vote 'ratings'" : '') . "
                    FROM " . DB_COMMENTS . " c
