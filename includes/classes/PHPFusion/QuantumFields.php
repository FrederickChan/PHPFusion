...
class QuantumFields extends QuantumFactory {
     ...
     */
    protected $plugin_folder = INCLUDES.'user_fields/';

    protected $plugin_locale_folder = LOCALE.LOCALESET.'user_fields/';
    ...
    public function returnFieldsInput($db, $primary_key, $callback_data = FALSE) {
        ...
        $section = get('section');
        $cur_section = $sec_id;
        if ($section && in_array($section, array_values($this->field_cat_index[0]))) {
            $cur_section = $section;
        }
        ...
        foreach ($field as $field_data) {
            ...
            $target_database = $field_data['field_cat_db'] ? DB_PREFIX.ltrim($field_data['field_cat_db'], DB_PREFIX) : $db;

            $col_name = !empty($field_data['field_cat_index']) ? $field_data['field_cat_index'] : $primary_key;

            // Find index primary key value
            $primaryKeyVal = sanitizer($col_name, (string)$field_data['field_default'], $col_name);
            ...
            if (check_post($field_data['field_name'])) {
                post($field_data['field_name']);
                //                if (check_post($field_data['field_name']) { // array support?
                $output_fields[$target_database][$field_data['field_name']] = sanitizer($field_data['field_name'], (string)$field_data['field_default'], $field_data['field_name']);
            }
        }
