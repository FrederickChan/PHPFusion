...
// lets refactor this format to core without going thru this. we will run a new core?
// because 1. the format of editing password is using modal.
// because 2. the format of browsing the page is custom as well.

function account_form($info) {
    ...
    $html .= '<p>Change user account passwords.</p>';
    ...
    if ($info['user_admin_password']) {
        $html .= fusion_get_function('openside', $locale['u130']);
        $html .= '<p>Change administrator account passwords.</p>';
...
function privacy_form($info) {
    ...
    if ($view = get('d')) {
        ...
        function display_login_activity($info) {
            ...
            $html .= '<p>The information listed are logged sessions that has accessed your account.</p>';
        ...
        $html .= '<p><a href="'.clean_request('', ['d'], FALSE).'">'.show_icon('left', 'me-1', $locale['back']).$locale['back'].'</a></p>';

        function privacy_display_control($view, $info) {
            switch ($view) {
                case 'twostep' :
                    return display_two_step($info);
                case 'records':
                    return display_login_activity($info);
                case 'data':
                    return display_data_activity($info);
                case 'login':
                    return display_social_logins($info);
            }
            return 'Content not found';
        }

        $html .= privacy_display_control($view, $info);
        ...
        $html .= '<a href="'.$info['twostep_url'].'" class="ms-auto btn btn-sm btn-default">Setup</a>';
        $html .= '</div>';
        $html .= '<div class="list-group-item d-flex align-items-center">';
        $html .= '<h6>Login activity</h6>';
        $html .= '<a href="'.$info['records_url'].'" class="ms-auto btn btn-sm btn-default">View</a>';
        $html .= '</div>';
        $html .= '<div class="list-group-item d-flex align-items-center">';
        $html .= '<h6>Manage your data and activity</h6>';
        $html .= '<a href="'.$info['data_url'].'" class="ms-auto btn btn-sm btn-default">View</a>';
        $html .= '</div>';
        $html .= '<div class="list-group-item d-flex align-items-center">';
        $html .= '<h6>Social Login</h6>';
        $html .= '<a href="'.$info['login_url'].'" class="ms-auto btn btn-sm btn-default">Bindings</a>';
        $html .= '</div>';
        $html .= '</div>';
    }
...
function notification_form($info) {
    $locale = fusion_get_locale();
    ...
    $html .= opencollapsebody($locale['u510'].'<p>'.$locale['u511'].'</p>', 'x1', 'email', TRUE);
